// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.2.5 低耦合优化版
// @namespace    http://tampermonkey.net/
// @version      6.2.5
// @description  修复Bug、日志通道分离、工具函数参数注入、进一步降低耦合性。
// @author       AI Combined
...
/**
 * 6.2.5 更新说明：
 * 1. 【Bug修复】修复第1467行 resetState() 函数调用错误，改为 resetAllStates()
 * 2. 【新增】StatusbarUI.createLogChannel() 日志通道分离，各模块独立日志标识
 * 3. 【优化】工具函数参数注入模式，降低隐式依赖
 * 4. 【优化】入口判定分离，支持独立启用/停用模块
 */
...
(function() {
    ...
    class StatusbarUI {
        ...
        injectStyles() {
            GM_addStyle(`
                ...
                /* 模块标签样式 */
                .ai-log-module-normal { color: #3370ff; font-weight: 500; }
                .ai-log-module-mcgg { color: #f5a623; font-weight: 500; }
                .ai-log-module-task { color: #fda085; font-weight: 500; }
            `);
        }
    ...
    (function() {
        ...
        // ===================== 独立日志通道 =====================
        const normalLogger = UI.createLogChannel('normal');
        const mcggLogger = UI.createLogChannel('mcgg');
        ...
        // ===================== 调试日志（使用独立通道） =====================
        function log(...args) {
            if (CONFIG_COMMON.debug) {
                const msg = args.join(' ');
                console.log('[工单助手]', ...args);
                normalLogger.log(msg);
            }
        }

        function logError(...args) {
            const msg = args.join(' ');
            console.error('[工单助手 错误]', ...args);
            normalLogger.error(msg);
        }

        function logMCGG(msg, type = 'info') {
            if (!CONFIG_COMMON.debug) return;
            if (type === 'error') {
                console.error(`[MCGG模块] ${msg}`);
                mcggLogger.error(msg);
            } else if (type === 'warn') {
                console.warn(`[MCGG模块] ${msg}`);
                mcggLogger.warn(msg);
            } else if (type === 'success') {
                console.log(`[MCGG模块] ${msg}`);
                mcggLogger.success(msg);
            } else {
                console.log(`[MCGG模块] ${msg}`);
                mcggLogger.log(msg);
            }
        ...
        function extractInternalDescription() {
            ...
            normalLogger.success(`提取内部描述成功，长度: ${extracted.length}`);
            return extracted;
        }
        ...
        async function extractInternalDescriptionWithRetry() {
            ...
            const maxRetries = Math.max(0, CONFIG_COMMON.internalDescMaxRetries || 0);
            for (let i = 0; i < maxRetries; i++) {
                log(`内部描述未就绪，${CONFIG_COMMON.internalDescRetryDelay}ms 后重试 (${i + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, CONFIG_COMMON.internalDescRetryDelay));
        ...
        function resetMCGGState() {
            mcggState = {
                currentTicketID: mcggState.currentTicketID, // 保留当前ID
                copiedText: '',
                leftHeading: '',
                versionNumber: '',
                channelText: '',
        ...
        async function extractMCGGInternalDescriptionWithRetry() {
            ...
            const maxRetries = Math.max(1, CONFIG_MCGG.mcggInternalDescMaxRetries || 0);
            for (let i = 0; i < maxRetries; i++) {
                logMCGG(`描述未就绪，${CONFIG_MCGG.mcggInternalDescRetryDelay}ms 后重试 (${i + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, CONFIG_MCGG.mcggInternalDescRetryDelay));
        ...
        function extractMCGGFaxiandiedai(heading) {
            return SharedUtils.extractVersion(heading);
        }
        ...
        function determineMCGGHeading(text) {
            ...
            mcggState.leftHeading = isTestServer ? CONFIG_MCGG.mcggtestServer : CONFIG_MCGG.mcggfullServer;
            mcggState.versionNumber = SharedUtils.extractVersion(mcggState.leftHeading);
        ...
        async function processMCGGTitleWithRetry() {
            ...
            try {
                while (Date.now() - startTime < CONFIG_MCGG.mcggTitleMaxWaitTime) {
                    ...
                    await new Promise(resolve => setTimeout(resolve, CONFIG_MCGG.mcggTitleRetryDelay));
                }
            } finally {
        ...
        async function handleMCGGChannelFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch(mcggState.channelText, mcggLogger);
            if (success) {
        ...
        async function handleMCGGIterationFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch(mcggState.faxiandiedai, mcggLogger);
            if (success) {
        ...
        async function handleMCGGModuleFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch('模式独立包', mcggLogger);
            if (success) {
        ...
        function determineHeading(text) {
            ...
            if (matches.length > 1) {
                ...
                normalLogger.warn(`检测到多个ServerID(${matches.length}个)，使用第一个`);
            }
            ...
            state.leftHeading = isTestServer ? CONFIG_NORMAL.testServer : CONFIG_NORMAL.fullServer;
            ...
            normalLogger.success(`识别环境: ${state.channelText}, 版本: ${state.versionNumber}, 迭代: ${state.faxiandiedai}`);
            return true;
        }
        ...
        async function processTitleWithRetry() {
            ...
            try {
                while (Date.now() - startTime < CONFIG_COMMON.titleMaxWaitTime) {
                    const input = SharedUtils.findTitleInputRobust();
                    ...
                    if (input) {
                        ...
                        if (!colonMatch) {
                            ...
                            if (contentPart && !SharedUtils.hasChinese(contentPart)) {
                                ...
                                if (CONFIG_COMMON.removeTrailingPunctuation) {
                    ...
                    await new Promise(resolve => setTimeout(resolve, CONFIG_COMMON.titleRetryDelay));
                }
            } finally {
        ...
        function translateViaGoogle(text) {
            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    ...
                    timeout: CONFIG_COMMON.translateTimeout,
                    onload: (response) => {
                        try {
        ...
        async function translateText(text) {
            if (state.translateCount >= CONFIG_COMMON.translateDailyLimit) {
                log('已达翻译次数上限');
                return text;
            }
                ...
                try {
                    ...
                    const result = await Promise.race([
                        ...
                        new Promise((_, reject) =>
                            setTimeout(() => reject(new Error('timeout')), CONFIG_COMMON.translateTimeout)
                        )
                    ]);
        ...
        async function handleChannelFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch(state.channelText, normalLogger);
        ...
        async function handleIterationFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch(state.faxiandiedai, normalLogger);
        ...
        function monitorTicketChange() {
            setInterval(() => {
                ...
                if (newTicketID && newTicketID !== state.currentTicketID) {
                    ...
                    normalLogger.success(`检测到新工单: ${newTicketID}`);
                    resetAllStates(); // ✅ 修复：使用正确的函数名
            ...
            }, CONFIG_COMMON.checkInterval);
        }

        // ===================== 初始化 =====================
        function init() {
            log('========================================');
            log('工单助手 v6.2.5 已启动 (低耦合优化版)');
            log('调试模式:', CONFIG_COMMON.debug);
            log('========================================');
    ...
    // =========================================================================
    // 脚本 B：AiHelp Task 客服信息提取一键复制（低耦合版）
    // =========================================================================
    (function() {
        'use strict';
        ...
        // ===================== 独立日志通道 =====================
        const taskLogger = UI.createLogChannel('task');
        ...
        async function handleCopyAction(button) {
            ...
            try {
                ...
                if (!finalAgentName) {
                    ...
                    taskLogger.success(`已复制: ${copyText}`);
                }
            } catch (e) {
