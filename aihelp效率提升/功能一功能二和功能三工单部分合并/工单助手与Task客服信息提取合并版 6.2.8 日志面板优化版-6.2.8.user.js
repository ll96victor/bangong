// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.2.8 日志面板优化版
// @namespace    http://tampermonkey.net/
// @version      6.2.8
// @description  日志面板优化版：异常加载检测 + 翻译日志对照 + 文本可选中 + 防抖机制 + 内存管理
 ...
 *
 * 【逻辑优化】
 * 1. 异常加载检测机制：监控"提取内部描述成功"的条目
 *    - 若提取内容长度 < 3，判定为网页未正确加载
 *    - 自动触发重新提取逻辑，避免后续处理空数据
 *
 * 【功能增强】
 * 2. 完善翻译日志显示：
 *    - 翻译成功后显示"原文"与"译文"对照
 *    - 超过50字符自动截断显示
 * 3. 改进用户交互体验：
 *    - 日志面板CSS设置 user-select: text
 *    - 允许用户选中和复制日志内容
 *
 * 【逻辑排查与优化】
 * 4. 防抖机制优化：
 *    - 添加 debounceDelay (300ms) 防抖间隔
 *    - 普通工单和MCGG模块均实现防抖跳过逻辑
 *    - 基于 lastProcessTime 时间戳判断
 *
 * 【内存管理】
 * 5. 日志内存优化：
 *    - 添加 logData 数组存储日志数据
 *    - 定期清理机制 (60秒间隔)
 *    - 当日志超过80%上限时自动清理30%旧日志
 *    - 提供 clearAllLogs() 方法手动清理
 *
 * 【保留架构优势】
        ...
        bindEvents() {
            ...
            const handleMouseDown = (e) => {
                ...
                const handleMouseMove = (moveEvent) => {
                    ...
                    if (this.isDragging) {
                        ...
                        newX = Math.max(0, Math.min(newX, window.innerWidth - (this.isExpanded ? 320 : 44)));
                        newY = Math.max(0, Math.min(newY, window.innerHeight - (this.isExpanded ? 300 : 44)));
            ...
            Object.keys(this.zones).forEach(zoneName => {
                ...
                zone.addEventListener('mouseenter', () => {
                    console.log('[UI] mouseenter:', zoneName);
                    this.startDelayedTipTimer(zoneName);
                });

                zone.addEventListener('mouseleave', () => {
                    console.log('[UI] mouseleave:', zoneName);
        ...
        collapse() {
            ...
            setTimeout(() => {
                if (!this.isExpanded) {
                    ...
                    // 强制重置所有zone的transform状态，避免尺寸变化问题
    ...
    (function() {
        ...
        function extractInternalDescription() {
            ...
            if (extracted.length < 3) {
                log('警告：提取内容长度异常（<3），可能网页未正确加载');
                logger.warn('提取内容长度异常（' + extracted.length + '），可能网页未正确加载');
                return '';
            }
