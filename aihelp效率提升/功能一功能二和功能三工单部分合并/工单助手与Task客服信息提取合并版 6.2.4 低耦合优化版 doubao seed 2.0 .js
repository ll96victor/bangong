// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.2.4 低耦合优化版
// @namespace    http://tampermonkey.net/
// @version      6.2.4
// @description  低耦合优化版：普通工单与MCGG模块完全解耦，独立配置、独立状态、独立入口、独立日志。
...
/**
 * 低耦合重构说明：
 * 1. 模块隔离：普通工单模块、MCGG模块、Task信息提取模块分别封装为独立IIFE
 * 2. 配置分离：各模块独立配置，互不干扰
 * 3. 状态隔离：各模块独立状态管理
 * 4. 入口分离：各模块独立监听和处理入口
 * 5. 日志区分：各模块日志前缀明确区分
 * 6. UI共用但逻辑分离：共享UI容器但按钮和日志独立管理
 */
...
(function() {
    ...
    const SharedUtils = {
        ...
        extractVersion: function(text) {
            const match = text.match(/(\d+(?:\.\d+)+)/);
            return match ? match[1] : '';
        },
        extractContentWithImages: function(element) {
        ...
        simulateInputValue: function(element, text) {
        ...
        findLabelText: function(targetInput) {
    ...
    const DropdownUtils = {
        waitForDropdownSearchInput: function(timeout = 1200) {
        ...
        fillDropdownSearch: async function(text, logFn) {
            const searchInput = await this.waitForDropdownSearchInput();
            if (!searchInput) {
                if (logFn) logFn('未找到下拉搜索框');
                return false;
            }
            try {
                ...
                if (logFn) logFn('下拉框填充成功: ' + text);
                return true;
            } catch (e) {
                if (logFn) logFn('下拉框填充失败: ' + e);
                return false;
            }
        }
    };
    ...
    (function() {
        ...
        function log(...args) {
            if (CONFIG.debug) {
                const msg = '[普通工单] ' + args.join(' ');
                console.log('[普通工单 v6.2.4]', ...args);
                UI.addLog(msg, 'info');
            }
        }
        ...
        function logError(...args) {
            const msg = '[普通工单] ' + args.join(' ');
            console.error('[普通工单 v6.2.4 错误]', ...args);
            UI.addLog(msg, 'error');
        }

        function debugLog(msg, type = 'info') {
            if (!CONFIG.debug) return;
            UI.addLog('[普通工单] ' + msg, type);
        }
        ...
        function findTitleInputRobust() {
            ...
            if (byPlaceholder && SharedUtils.isInputAvailable(byPlaceholder)) {
                        ...
                        if (container) {
                            ...
                            while (sibling) {
                                if (sibling.classList && sibling.classList.contains('detail')) {
                                    ...
                                    if (input && SharedUtils.isInputAvailable(input)) {
                                        return input;
                                    }
                                }
                                sibling = sibling.nextElementSibling;
                            ...
                            if (fallback && SharedUtils.isInputAvailable(fallback)) {
                                return fallback;
                            }
                        }
                    }
        ...
        async function handleChannelFocus() {
            ...
            const success = await DropdownUtils.fillDropdownSearch(state.channelText, log);
        ...
        async function handleIterationFocus() {
            ...
            const success = await DropdownUtils.fillDropdownSearch(state.faxiandiedai, log);
    ...
    (function() {
        ...
        function determineMCGGHeading(text) {
            ...
            state.versionNumber = SharedUtils.extractVersion(state.leftHeading);
            state.channelText = isTestServer ? '测服' : '全服';
            state.faxiandiedai = extractMCGGFaxiandiedai(state.leftHeading);
        ...
        async function handleMCGGModuleFocus() {
            ...
            const success = await DropdownUtils.fillDropdownSearch('模式独立包', log);
            if (success) {
