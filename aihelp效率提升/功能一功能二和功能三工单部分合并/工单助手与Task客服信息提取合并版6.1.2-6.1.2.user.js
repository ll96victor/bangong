// ==UserScript==
// @name         工单助手与Task客服信息提取合并版6.1.2
// @namespace    http://tampermonkey.net/
// @version      6.1.2
// @description  合并功能：1. 自动翻译、内部描述复制、下拉框填充；2. Task 客服信息提取一键复制。完全保留原版代码逻辑，仅合并 UI。
// @author       AI Combined
...
/**
 * 冲突处理说明：
 * 1. 逻辑 100% 还原：不再对脚本 A 进行任何代码压缩或日志剥离，保留全部 800+ 行原始逻辑。
 * 2. 共享容器：脚本 A 和 B 的按钮现在通过 initSharedUI() 挂载到同一个容器。
 * 3. 统一拖拽：实现了一套不干扰内部点击的拖拽逻辑。
 */
...
(function() {
    ...
    function initSharedUI() {
        let container = document.getElementById('merged-helper-container');
        if (container) return container;

        container = document.createElement('div');
        container.id = 'merged-helper-container';
        ...
        container.addEventListener('mousedown', (e) => {
            // 点击按钮时不触发拖拽
            if (e.target.closest('#copy-status-box') || e.target.closest('#aihelp-task-copy')) return;
    ...
    GM_addStyle(`
        ...
        #debug-console h3 { margin: 0 0 8px 0; color: white; font-size: 13px; border-bottom: 1px solid #666; padding-bottom: 4px; display: flex; justify-content: space-between; }
        #debug-console button { background: #333; color: white; border: 1px solid #666; cursor: pointer; padding: 1px 4px; font-size: 10px; }
    ...
    (function() {
        ...
        function initCopyButton() {
            const container = initSharedUI();
            if (document.getElementById('aihelp-task-copy')) return;
