// ==UserScript==
// @name         工单助手 - 自动翻译与内部描述复制 v5.5 KIMI 2.5 Agent优化版
// @namespace    http://tampermonkey.net/
// @version      5.5
// @description  【优化版】结合4.2精准填充逻辑与5.4内存优化，实现需求四/五精准填充
// @author       ll96victor (Optimized v5.5)
...
(function() {
    ...
    function log(...args) {
        if (CONFIG.debug) {
            console.log('[工单助手 v5.5]', ...args);
        }
    }

    function logError(...args) {
        console.error('[工单助手 v5.5 错误]', ...args);
    }
    ...
    function createDebugConsole() {
        ...
        box.innerHTML = `
            <h3>脚本运行日志 v5.5 <button onclick="this.parentElement.parentElement.style.display='none'">关闭</button></h3>
    ...
    // ===================== 下拉框填充模块（核心修复：采用4.2版本的focus监听逻辑） =====================
    ...
    async function processTicket() {
        ...
        try {
            ...
            // 步骤5：设置自动填充监听（不阻塞，采用4.2的focus监听方式）
            autoFillChannel();
            autoFillIteration();
