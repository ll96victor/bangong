// ==UserScript==
// @name         工单助手 - 自动翻译与内部描述复制 v5.9.6 修复中文重复翻译
// @namespace    http://tampermonkey.net/
// @version      5.9.6
// @description  【修复重复】修正包含中文时的拼接逻辑，解决内容重复问题
// @author       ll96victor (Optimized v5.9.6)
...
(function() {
    ...
    function log(...args) {
        if (CONFIG.debug) {
            console.log('[工单助手 v5.9.6]', ...args);
        }
    }

    function logError(...args) {
        console.error('[工单助手 v5.9.6 错误]', ...args);
    }
    ...
    function createDebugConsole() {
        ...
        box.innerHTML = `
            <h3>脚本运行日志 v5.9.6 <button onclick="this.parentElement.parentElement.style.display='none'">关闭</button></h3>
        ...
        while (Date.now() - startTime < CONFIG.titleMaxWaitTime) {
            ...
            if (input) {
                ...
                if (!colonMatch) {
                    ...
                    // 区分拼接逻辑：如果有翻译结果，则拼接"翻译+原文"；否则只拼接"原文"
                    let newTitle;
