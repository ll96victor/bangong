// ==UserScript==
// @name         工单助手 - 自动翻译与内部描述复制 v5.9.8 纯净优化版
// @namespace    http://tampermonkey.net/
// @version      5.9.8
// @description  【纯净优化】完全保留原版 800+ 行稳定逻辑，仅修复异步翻译重叠问题。仅在 processTitleWithRetry 函数中加入一个极其微小的“异步锁”，以解决你之前可能遇到的网络慢时标题重复翻译的小概率问题
// @author       ll96victor (Optimized v5.9.8)
...
(function() {
    ...
    function init() {
        ...
        const isTargetPage = currentUrl.includes('task?orderId') || currentUrl.includes('tasks?searchType');
        
        // 如果不是目标页面，直接退出，不执行任何代码
        if (!isTargetPage) {
            // 如果需要，可以取消下面这行注释以便在控制台确认脚本已跳过
            // console.log('[工单助手] 当前页面非 Task 页面，脚本停止运行。');
            return;
        }
        // ------------------------
