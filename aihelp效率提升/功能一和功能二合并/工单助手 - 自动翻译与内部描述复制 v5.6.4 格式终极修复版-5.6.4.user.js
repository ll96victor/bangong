// ==UserScript==
// @name         工单助手 - 自动翻译与内部描述复制 v5.6.4 格式终极修复版
// @namespace    http://tampermonkey.net/
// @version      5.6.4
// @description  【终极修复版】修正v5.6.2换行丢失问题，采用“临时DOM挂载”方案，完美还原原格式（支持多段落、图片转链接）。5.6.4直接替换 v5.6.3 完整代码中的 extractContentWithImages 这个函数
// @author       ll96victor (Ultimate Fix v5.6.3)
...
(function() {
    ...
    function log(...args) {
        if (CONFIG.debug) {
            console.log('[工单助手 v5.6.3]', ...args);
        }
    }

    function logError(...args) {
        console.error('[工单助手 v5.6.3 错误]', ...args);
    }
    ...
    function createDebugConsole() {
        ...
        box.innerHTML = `
            <h3>脚本运行日志 v5.6.3 <button onclick="this.parentElement.parentElement.style.display='none'">关闭</button></h3>
    ...
    function extractInternalDescription() {
        ...
        // 方式1：收集父元素下"内部描述"与"描述"之间的所有兄弟元素
        ...
        return extracted;
            ...
            // 提取内容并处理图片（采用TreeWalker手动拼接，图片保留在原位置）
    function extractContentWithImages(element) {
        ...
        // 2. 使用 TreeWalker 遍历文本节点并手动拼接换行符
        ...
        // 3. 清理开头可能的标签文本（如果有）
        result.text = result.text.replace(/^(内部描述[\*\s]*[：:]?\s*)/i, '');
    ...
    function determineHeading(text) {
        ...
        // 优化：优先取第一个有效的ServerID，不再强制限制只能有1个（防止克隆节点导致重复匹配）
        if (matches.length === 0) {
            log(`未找到ServerID`);
            debugLog(`ServerID不符合要求，未匹配到`, 'warn');
            return false;
        }
        // 如果有多个，取第一个，但记录警告
        if (matches.length > 1) {
            log(`警告：检测到多个ServerID，将使用第一个: ${matches[0]}`);
