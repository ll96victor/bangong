// ==UserScript==
// @name         工单助手 - 自动翻译和标记标题 v1.3.4（最终修复版）
// @namespace    http://tampermonkey.net/
// @version      1.3.4
// @description  恢复发现迭代自动点击 + 确保创建人逻辑生效 + 彻底移除经办人。在1.3版本可以自动翻译并改标题冒号前面的迭代的基础上，增加了发现迭代提取和自动点击，创建人固定并点击。
// @author       ll96victor
...
(function() {
    ...
    const CONFIG = {
        ...
        checkInterval: 2000,
        maxRetries: 3,
        clickDelay: 1000,  // 延长点击延迟，适配下拉加载
        searchDelay: 1200  // 延长搜索延迟，确保选项加载
    };
    ...
    // 优化findElementByText：精准匹配创建人，包含匹配发现迭代
    ...
    async function selectSearchableDropdown(labelText, searchText, isCreator = false) {
        ...
        for (const labelElement of labelElements) {
            ...
            // 额外等待：确保下拉选项加载完成（核心修复）
            await new Promise(resolve => setTimeout(resolve, 800));

            // 步骤4: 查找匹配选项（核心修复：恢复包含匹配）
            log('步骤4: 查找匹配的选项');
            ...
            // 收集匹配的选项：创建人用包含匹配，发现迭代用包含匹配
            const matchedOptions = [];
            for (const option of dropdownOptions) {
                const optionText = option.textContent.trim();
                // 核心修复：发现迭代用包含匹配（适配2.1.52和2.1.52(C#)），创建人用包含匹配
            ...
            if (selected) {
                ...
                return 'await_manual'; // 恢复该返回值，适配监听逻辑
            } else {
                log(`✗ ${labelText} 处理失败`);
                return false;
            }
