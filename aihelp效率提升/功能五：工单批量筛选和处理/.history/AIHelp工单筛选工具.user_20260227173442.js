...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            console.log('[AIHelp工单工具] Step 1: 点击筛选按钮');
            await ToolUtil.clickByText(['button', 'span', 'i'], '筛选');
            await ToolUtil.sleep(500);
            console.log('[AIHelp工单工具] Step 1 完成');

            console.log('[AIHelp工单工具] Step 2: 点击重置按钮');
            await ToolUtil.clickByText(['button', 'span'], '重置');
            await ToolUtil.sleep(500);
            console.log('[AIHelp工单工具] Step 2 完成');

            // 滚动弹窗内容到底部
            console.log('[AIHelp工单工具] Step 3: 滚动弹窗');
            ...
            for (const selector of scrollSelectors) {
                ...
                for (const container of containers) {
                    if (container.scrollHeight > container.clientHeight) {
                        ...
                        console.log('[AIHelp工单工具] 滚动容器:', selector, '高度:', container.scrollHeight);
                    }
                }
            }
            await ToolUtil.sleep(800);
            console.log('[AIHelp工单工具] Step 3 完成');

            // 查找"包含其中任一标签"文字
            console.log('[AIHelp工单工具] Step 4: 查找"包含其中任一标签"');
            ...
            for (const div of allDivs) {
                ...
                if (text && text.includes('包含其中任一标签') && !text.includes('工单分类') && !text.includes('包含所有标签')) {
                    // 确保这个元素可见
                    const rect = div.getBoundingClientRect();
                    if (rect.width > 0 && rect.height > 0) {
                        labelContainer = div;
                        console.log('[AIHelp工单工具] 找到"包含其中任一标签"容器, 文本:', text.substring(0, 50));
                        break;
                    }
                }
            }
            ...
            if (!labelContainer) {
                console.log('[AIHelp工单工具] 未找到"包含其中任一标签"容器，尝试其他方式');
            }

            // 查找所有标签输入框
            const allInputs = document.querySelectorAll('input[placeholder="请选择标签"]');
            console.log('[AIHelp工单工具] 找到标签输入框数量:', allInputs.length);
            ...
            for (let i = 0; i < allInputs.length; i++) {
                ...
                for (let j = 0; j < 5 && parent; j++) {
                    ...
                    if (prevSibling) {
                        nearbyText = prevSibling.textContent.trim().substring(0, 30);
                        break;
                    }
                    parent = parent.parentElement;
                }
                console.log(`[AIHelp工单工具] 输入框${i}: 位置top=${rect.top}, 附近文字="${nearbyText}"`);
            }

            // 选择正确的输入框（"包含其中任一标签"通常是最后一个）
            let targetInput = null;
            if (allInputs.length >= 3) {
                // 如果有3个或更多输入框，选择最后一个
                targetInput = allInputs[allInputs.length - 1];
                console.log('[AIHelp工单工具] 选择最后一个标签输入框');
            } else if (allInputs.length > 0) {
                targetInput = allInputs[allInputs.length - 1];
                console.log('[AIHelp工单工具] 选择最后一个标签输入框(备选)');
            }
            ...
            // 滚动到目标输入框并点击
            console.log('[AIHelp工单工具] Step 5: 点击标签输入框');
            targetInput.scrollIntoView({ behavior: 'smooth', block: 'center' });
            await ToolUtil.sleep(300);
            targetInput.click();
            console.log('[AIHelp工单工具] 已点击标签输入框');
            await ToolUtil.sleep(500);

            // 输入搜索内容
            console.log('[AIHelp工单工具] Step 6: 输入搜索内容');
            ...
            if (searchInput) {
                ...
                console.log('[AIHelp工单工具] 已输入搜索内容');
            } else {
                console.log('[AIHelp工单工具] 未找到搜索输入框，尝试当前激活元素');
                ...
                if (activeInput && activeInput.tagName === 'INPUT') {
                    ...
                    console.log('[AIHelp工单工具] 已在激活元素输入');
                }
            }
            await ToolUtil.sleep(800);

            // 点击下拉选项
            console.log('[AIHelp工单工具] Step 7: 点击下拉选项');
            await ToolUtil.clickByText(['li', 'span'], 'AI识别为BUG bug identified by ai');
            await ToolUtil.sleep(500);

            console.log('[AIHelp工单工具] Step 8: 点击筛选按钮');
            await ToolUtil.clickFilterButton();
            await ToolUtil.sleep(500);

            console.log('[AIHelp工单工具] 所有步骤完成');
