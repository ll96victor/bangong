...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            // Step 4: 通过标签文字定位输入框
            // 查找包含"包含其中任一标签"文字的label或div
            const allLabels = document.querySelectorAll('label, div.el-form-item__label, div');
            let targetInput = null;
            
            for (const label of allLabels) {
                const text = label.textContent || '';
                // 精确匹配"包含其中任一标签"，排除其他
                if (text.trim() === '包含其中任一标签' || (text.includes('包含其中任一标签') && !text.includes('工单分类') && !text.includes('包含所有标签'))) {
                    // 找到了标签，现在找它对应的输入框
                    // 通常输入框在标签的下一个兄弟元素或父元素的下一个子元素中
                    let parent = label.parentElement;
                    
                    // 向上查找form-item
                    for (let i = 0; i < 5 && parent; i++) {
                        // 在父元素中查找输入框
                        const input = parent.querySelector('input[placeholder="请选择标签"]');
                        if (input) {
                            // 确保这个输入框是可见的
            ...
            if (!targetInput) {
                ...
                for (const input of allInputs) {
                    const rect = input.getBoundingClientRect();
                    if (rect.width > 0 && rect.height > 0 && rect.top > 0) {
                        visibleInputs.push({ input, top: rect.top });
                    }
                }
                
                // 按位置排序，选择最下面的（"包含其中任一标签"通常在最下面）
                if (visibleInputs.length > 0) {
                    visibleInputs.sort((a, b) => b.top - a.top);
                    targetInput = visibleInputs[0].input;
                }
            }
            ...
            for (const li of listItems) {
                if (li.textContent.includes('AI识别为BUG bug identified by ai')) {
                    li.click();
                    break;
                }
            }
