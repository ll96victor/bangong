...
(function() {
    ...
    const TipManager = {
        ...
        startTimer(btnId, showTipCallback) {
            this.cancelTimer(btnId);
            this.timers[btnId] = setTimeout(() => {
                showTipCallback();
            }, 3000);
        },
    ...
    function createToolbar() {
        ...
        logBtn.addEventListener('mouseenter', () => {
            TipManager.startTimer(logBtn.id, () => {
                TipManager.showTip(logBtn, `
                    <div class="tip-title">查看操作日志</div>
                    <div class="tip-desc">显示所有筛选和批量操作的执行记录</div>
                `);
            });
        });

        logBtn.addEventListener('mouseleave', () => {
            TipManager.cancelTimer(logBtn.id);
            TipManager.hideTip(logBtn);
        });
