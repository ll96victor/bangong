...
(function() {
    ...
    const ToolUtil = {
        ...
        async clickFilterButton() {
            ...
            // 方案1：查找弹窗内的主按钮（el-button--primary）
            const dialogs = document.querySelectorAll('.el-dialog, .el-drawer, .el-popover');
            console.log('[AIHelp工单工具] 找到弹窗数量:', dialogs.length);
            
            for (const dialog of dialogs) {
                if (!this.isElementAvailable(dialog)) continue;
                
                const primaryBtns = dialog.querySelectorAll('button.el-button--primary');
                console.log('[AIHelp工单工具] 弹窗内主按钮数量:', primaryBtns.length);
                
                for (const btn of primaryBtns) {
                    const text = btn.textContent.trim();
                    console.log('[AIHelp工单工具] 按钮文本:', text);
                    if (text === '筛选') {
            ...
            for (const btn of allButtons) {
                const text = btn.textContent.trim();
                if (text === '筛选' && this.isElementAvailable(btn)) {
            ...
            // 方案3：直接查找span包含"筛选"的按钮
            const spans = document.querySelectorAll('span');
            for (const span of spans) {
                if (span.textContent.trim() === '筛选') {
                    const parentBtn = span.closest('button');
                    if (parentBtn && this.isElementAvailable(parentBtn)) {
