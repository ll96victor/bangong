...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            await ToolUtil.sleep(800); // 等待下拉列表出现

            // Step 6: 输入搜索内容
            console.log('Step 6: 输入搜索内容');
            
            // 查找下拉列表中的搜索框 - 必须是可见的
            const allSearchInputs = document.querySelectorAll('input.elp-cascader__search-input');
            ...
            for (const input of allSearchInputs) {
                ...
                if (rect.width > 0 && rect.height > 0) {
                    console.log('搜索框位置:', rect.top, rect.left);
                    // 选择位置最接近目标输入框的搜索框
                    if (!searchInput || Math.abs(rect.top - targetInput.getBoundingClientRect().top) < 200) {
                        searchInput = input;
                    }
                }
            }
            ...
            if (searchInput) {
                ...
                console.log('已输入到目标输入框');
            }
            await ToolUtil.sleep(1000); // 等待搜索结果

            // Step 7: 点击下拉选项
            console.log('Step 7: 点击下拉选项');
            await ToolUtil.sleep(500); // 等待下拉列表加载
            ...
            for (const li of listItems) {
                ...
                if (rect.width > 0 && rect.height > 0) {
                    ...
                    if (text.includes('AI识别为BUG') || text.includes('bug identified by ai')) {
                        li.click();
                        foundOption = true;
                        console.log('已点击下拉选项:', text.substring(0, 50));
                        break;
                    }
                }
            }
            if (!foundOption) {
                console.log('未找到下拉选项，尝试滚动列表');
                // 尝试滚动下拉列表
                const dropdown = document.querySelector('.elp-cascader-menu__wrap, .vue-recycle-scroller');
                if (dropdown) {
                    dropdown.scrollTop = 0; // 滚动到顶部
                    await ToolUtil.sleep(500);
                }
                
                // 再次查找
                const listItems2 = document.querySelectorAll('li.elp-cascader-node, li');
                for (const li of listItems2) {
                    const text = li.textContent || '';
                    const rect = li.getBoundingClientRect();
                    if (rect.width > 0 && rect.height > 0 && (text.includes('AI识别为BUG') || text.includes('bug identified by ai'))) {
                        li.click();
                        foundOption = true;
                        console.log('已点击下拉选项(滚动后):', text.substring(0, 50));
                        break;
                    }
                }
            }
            if (!foundOption) {
                console.log('仍未找到下拉选项');
            }
            await ToolUtil.sleep(500);
