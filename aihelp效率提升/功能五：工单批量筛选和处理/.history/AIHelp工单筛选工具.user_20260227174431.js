...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            // 多次滚动确保到底
            for (let i = 0; i < 3; i++) {
                const scrollContainers = document.querySelectorAll('.el-dialog__body, .el-drawer__body, .el-scrollbar__wrap, [class*="scroll"]');
            ...
            // Step 4: 获取所有标签输入框，选择第3个（索引2）= "包含其中任一标签"
            const allInputs = document.querySelectorAll('input[placeholder="请选择标签"]');
            
            if (allInputs.length < 3) {
                throw new Error(`只找到${allInputs.length}个标签输入框，需要至少3个`);
            }
            
            // 输入框2 = "包含其中任一标签"
            const targetInput = allInputs[2];
