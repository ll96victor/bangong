...
(function() {
    ...
    const ActionE = {
        ...
        async execute() {
            ...
            // Step 3: 滚动弹窗到底部，让"发送奖励"选项可见
            console.log('Step 3: 滚动弹窗');
            const scrollContainers = document.querySelectorAll('.el-dialog__body, .el-drawer__body, .el-scrollbar__wrap');
            for (const container of scrollContainers) {
                if (container.scrollHeight > container.clientHeight) {
                    container.scrollTop = container.scrollHeight;
            ...
            const statusInput = document.querySelector('input[placeholder="请输入工单状态"]');
            if (statusInput) {
