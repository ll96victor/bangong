...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            // 查找并滚动弹窗内容，让"包含其中任一标签"选项可见
            console.log('[AIHelp工单工具] 开始滚动弹窗...');
            ...
            for (const el of allElements) {
                ...
                if (overflow.includes('auto') || overflow.includes('scroll')) {
                    if (el.scrollHeight > el.clientHeight) {
                        console.log('[AIHelp工单工具] 找到滚动容器:', el.className, 'scrollHeight:', el.scrollHeight);
            ...
            for (const input of labelInputs) {
                ...
                console.log('[AIHelp工单工具] 标签输入框位置:', rect.top, rect.left, rect.width, rect.height);
                
                // 如果输入框不在可见区域，滚动到可见
                if (rect.top < 100 || rect.top > window.innerHeight - 100) {
                    input.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    console.log('[AIHelp工单工具] 滚动到标签输入框');
                    await ToolUtil.sleep(500);
                }
            }

            // 点击标签输入框
            const labelInput = document.querySelector('input[placeholder="请选择标签"]');
            if (labelInput) {
