...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            // Step 4: 通过精确文字"包含其中任一标签"定位输入框
            let targetInput = null;
            
            // 方法1: 查找包含精确文字的元素，然后找它对应的输入框
            const allTextElements = document.querySelectorAll('div, span, label');
            for (const el of allTextElements) {
                const text = el.textContent.trim();
                // 精确匹配"包含其中任一标签"，排除其他类似文字
                if (text === '包含其中任一标签') {
                    // 找到了标签文字，现在向上查找父元素中的输入框
            ...
            // 方法2: 如果方法1失败，通过位置选择（选择top值最大的，即最下面的）
            if (!targetInput) {
            ...
            // Step 7: 点击下拉选项 - 查找包含"AI识别为BUG"的选项
