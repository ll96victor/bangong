...
(function() {
    ...
    const ActionA = {
        ...
        async execute() {
            ...
            // 查找"包含其中任一标签"文字，定位到正确的输入框
            console.log('[AIHelp工单工具] 查找"包含其中任一标签"输入框...');
            ...
            const allTextNodes = document.querySelectorAll('div, span, label, p');
            
            for (const el of allTextNodes) {
                if (el.textContent.includes('包含其中任一标签')) {
                    console.log('[AIHelp工单工具] 找到"包含其中任一标签"文字元素:', el.textContent.trim());
                    ...
                    for (let i = 0; i < 5 && parent; i++) {
                        ...
                        if (nextSibling) {
                            const input = nextSibling.querySelector('input[placeholder="请选择标签"]');
                            if (input) {
                                targetInput = input;
                                console.log('[AIHelp工单工具] 找到目标输入框(同级)');
                                break;
                            }
                        }
                        
                        // 查找父元素内部的input
                        const innerInput = parent.querySelector('input[placeholder="请选择标签"]');
                        if (innerInput) {
                            targetInput = innerInput;
                            console.log('[AIHelp工单工具] 找到目标输入框(内部)');
                            break;
                        }
            ...
            // 如果没找到，尝试其他方式
            if (!targetInput) {
                console.log('[AIHelp工单工具] 未通过文字找到输入框，尝试其他方式...');
                
                // 查找所有标签输入框，选择最后一个（通常是"包含其中任一标签"）
                const labelInputs = document.querySelectorAll('input[placeholder="请选择标签"]');
                console.log('[AIHelp工单工具] 找到标签输入框数量:', labelInputs.length);
                
                if (labelInputs.length > 0) {
                    targetInput = labelInputs[labelInputs.length - 1];
            ...
            targetInput.click();
            console.log('[AIHelp工单工具] 已点击目标输入框');
            await ToolUtil.sleep(500);
