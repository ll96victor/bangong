...
(function() {
    ...
    const ToolUtil = {
        ...
        async fastClick(element, options = {}) {
            const { needScroll = false, fastDelay = 100, fallbackDelay = 300 } = options;
        ...
        async fastFindAndClick(selector, options = {}) {
            const { needScroll = false, timeout = 5000, fastDelay = 100, fallbackDelay = 300 } = options;
        ...
        async fastClickByText(text, options = {}) {
            const { selectors = ['button', 'span', 'li'], needScroll = false, fastDelay = 100, fallbackDelay = 300 } = options;
    ...
    function createAssignAction(assigneeKey) {
        ...
        return {
            ...
            async execute() {
                ...
                assigneeInput.focus();
                await ToolUtil.fastClick(assigneeInput, { fastDelay: 100, fallbackDelay: 800 });
                ...
                if (foundOption) {
                    ...
                    await ToolUtil.fastClick(foundOption, { fastDelay: 100, fallbackDelay: 500 });
                ...
                if (submitBtn) {
                    ...
                    await ToolUtil.fastClick(submitBtn, { fastDelay: 100, fallbackDelay: 500 });
                    console.log('已点击提交按钮');
                } else {
    ...
    function createResolveAction() {
        ...
        return {
            ...
            async execute() {
                ...
                await ToolUtil.sleep(100);
                statusInput.focus();
                await ToolUtil.fastClick(statusInput, { fastDelay: 100, fallbackDelay: 800 });
            ...
            async execute() {
                ...
                for (const btn of filterBtns) {
                    if (btn.textContent.includes('筛选') && btn.querySelector('i.el-icon-search')) {
                        await ToolUtil.fastClick(btn, { fastDelay: 100, fallbackDelay: 800 });
                        break;
                    }
                }
                ...
                for (const btn of resetBtns) {
                    if (btn.textContent.trim() === '重置') {
                        await ToolUtil.fastClick(btn, { fastDelay: 100, fallbackDelay: 500 });
                        break;
                    }
                }
                ...
                await ToolUtil.sleep(100);
                targetInput.focus();
                await ToolUtil.fastClick(targetInput, { fastDelay: 100, fallbackDelay: 500 });
                ...
                if (foundStatusOption) {
                    ...
                    await ToolUtil.fastClick(foundStatusOption, { fastDelay: 100, fallbackDelay: 500 });
                } else {
                    logger.error(`未找到状态选项: ${config.status}`);
                    throw new Error(`未找到状态选项: ${config.status}`);
                }
                ...
                if (rewardInput) {
                    ...
                    await ToolUtil.fastClick(rewardInput, { fastDelay: 100, fallbackDelay: 800 });
                ...
                if (mailItem) {
                    ...
                    await ToolUtil.fastClick(mailItem, { fastDelay: 100, fallbackDelay: 500 });
                    console.log('已点击奖励选项');
                } else {
    ...
    const ActionE = {
        ...
        async execute() {
            ...
            if (foundQA) {
                ...
                await ToolUtil.fastClick(foundQA, { fastDelay: 100, fallbackDelay: 500 });
                console.log('已选择"= QA"');
            } else {
    ...
    function init() {
        ...
        createLogPanel();
        monitorRouteChange();
        console.log('[AIHelp工单工具] 已加载 v3.4.3 - 点击悬浮图标执行筛选或批量操作');
    }
