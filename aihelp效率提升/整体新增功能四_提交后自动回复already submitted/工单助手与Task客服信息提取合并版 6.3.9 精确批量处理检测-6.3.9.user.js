// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.3.9 精确批量处理检测
// @namespace    http://tampermonkey.net/
// @version      6.3.9
// @description  精确检测批量处理模式 只有当"编辑"按钮父元素类名包含el-button-group时才判定为批量处理
 ...
 *
 * 【修复问题】
 * 1. 修复"编辑"按钮检测逻辑，添加父元素类名判断
 *    - 只有当"编辑"按钮的父元素类名包含"el-button-group"时才判定为批量处理模式
 *    - 避免误判其他页面上普通的"编辑"按钮
 *
 * 2. 批量处理模式的判断条件（必须同时满足）：
 *    - 按钮文本完全等于"编辑"
 *    - 按钮可见（display !== 'none' && visibility !== 'hidden'）
 *    - 父元素类名包含"el-button-group"
 *
 * 【保留功能】
                ...
                if (clickableEl) {
                    ...
                    for (const btn of editButtons) {
                        ...
                        if (normalizedBtnText === '编辑') {
                            ...
                            if (style.display !== 'none' && style.visibility !== 'hidden' && parentClass.includes('el-button-group')) {
                                autoReplyLog('检测到"编辑"按钮存在（批量处理模式），跳过自动回复');
                                return;
                            }
                        }
