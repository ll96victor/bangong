// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.4.6
// @namespace    http://tampermonkey.net/
// @version      6.4.6
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：修复DeepLX公开镜像，预留GLM大模型翻译接口，精简失效接口
...
/**
 * 6.4.6 翻译接口修复与架构优化 更新说明：
 *
 * 【修复与调整】
 * 1. 修复 DeepLX 接口：
 *    - 切换为国内可稳定直连的公开镜像 `api.deeplx.fun`
 *    - 解决原官方接口导致的 unknown error
 * 2. 移除不稳定/受限接口：
 *    - 移除 Microsoft Edge 翻译（其非公开API容易被拦截导致 unknown error）
 * 3. 引入大模型翻译架构预留：
 *    - 增加基于 智谱 GLM-4-Flash（免费API）的翻译函数 `translateViaGLM4Flash`
 *    - 方便未来如果用户申请了免费API Key，可直接填入使用，提供媲美人工的翻译质量
 *
 * 【历史更新】
 * v6.4.5: 优化日志翻译面板，支持失败状态显示与对比
 * v6.4.4: 引入多源并发翻译与日志交互面板
 */
...
(function() {
    ...
    (function() {
        ...
        function translateViaGLM4Flash(text) {
            return new Promise((resolve, reject) => {
                // 如果用户愿意申请免费API Key，可以填在这里。默认使用一种公开分享的或者留空提示。
                // 鉴于用户暂时不想用API，此接口默认暂时作为保留扩展。
                // 但为了满足"添加免费AI翻译"需求，这里预留接口架构。
                const apiKey = ''; // 需要用户自行填入智谱API Key
                if (!apiKey) {
                    return reject(new Error('未配置API Key(智谱)'));
                }
