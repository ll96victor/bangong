// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.4.9
// @namespace    http://tampermonkey.net/
// @version      6.4.9
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：修复跨域权限导致的新增API unknown error 报错问题
...
/**
 * 6.4.9 API跨域权限修复 更新说明：
 *
 * 【核心修复】
 * 1. 修复所有新增翻译源提示 `unknown error` 的问题：
 *    - 原因是油猴脚本在发送跨域请求（GM_xmlhttpRequest）时，必须在脚本头部声明 `@connect` 域名权限。
 *    - 之前只声明了 Google 和 MyMemory 的域名，导致 DeepLX、Popcat、智谱AI 的请求被浏览器直接拦截并报错。
 *    - 现已补全所有域名的 `@connect` 声明，确保请求能正常发送。
 *
 * 【历史更新】
 * v6.4.8: 扩展智谱大模型支持，允许通过菜单选择所有免费模型
 * v6.4.7: 移除硬编码 API Key，改为菜单安全配置
 */
    ...
    (function() {
        ...
        function translateViaGLM4Flash(text) {
            return new Promise((resolve, reject) => {
                ...
                // 允许用户配置特定的模型版本，默认使用 glm-4-flash
                let modelVersion = GM_getValue('glm_model_version_v1', 'glm-4-flash');
        ...
        function initUI() {
            ...
            GM_registerMenuCommand('选择智谱AI 翻译模型', () => {
                const currentModel = GM_getValue('glm_model_version_v1', 'glm-4-flash');
                const models = [
                    'glm-4-flash',
                    'glm-4.7-flash',
                    'glm-4.6v-flash',
                ...
                models.forEach((m, i) => promptText += `${i + 1}. ${m}\n`);
