// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.5.4
// @namespace    http://tampermonkey.net/
// @version      6.5.4
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：优化Popcat错误检测，修复跨域权限导致的新增API unknown error 报错问题，优化GLM Prompt与后处理，延迟显示交互面板以便对比
 ...
 * 【优化与修复】
 * 1. 优化翻译面板展示时机：
 *    - 当有最快结果（如谷歌/MyMemory）返回并替换标题后，脚本会**延迟 5 秒**再显示多源交互面板
 *    - 这样给其他较慢的接口（如 Popcat、DeepLX、智谱AI）足够的缓冲时间
 *    - 确保面板出现时，用户能看到更多已经完成的翻译结果进行对比，而不是空白或加载中
 * 2. 增强翻译结果后处理：
 *    - 在面板展示逻辑中也同步应用了引号去除规则
 *
 * 【历史更新】
 * v6.5.3: 优化智谱 GLM 翻译后处理，自动去除引号
 * v6.5.2: 优化智谱 GLM 翻译 Prompt，调整默认模型
 */
