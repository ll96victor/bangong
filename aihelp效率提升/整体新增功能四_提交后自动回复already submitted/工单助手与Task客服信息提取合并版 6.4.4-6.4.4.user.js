// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.4.4
// @namespace    http://tampermonkey.net/
// @version      6.4.4
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：引入多源AI翻译并集成到日志交互面板中
...
(function() {
    ...
    (function() {
        ...
        function renderTranslationLogPanel(originalText, results) {
            ...
            let selectedText = results[0].text;
            ...
            results.forEach((r, idx) => {
                ...
                label.style.cssText = 'font-weight: 600; color: #3370ff; width: 65px; flex-shrink: 0; font-size: 10px; text-align: right;';
                ...
                if (successful.length > 0) {
                    ...
                    for (const r of successful) {
                        ...
                        if (!seen.has(cleanText)) {
                            seen.add(cleanText);
                            uniqueResults.push({ name: r.name, text: cleanText });
                        }
                    }

                    if (uniqueResults.length > 0) {
                        log('生成多源翻译交互面板');
                        renderTranslationLogPanel(text, uniqueResults);
                    }
                } else {
