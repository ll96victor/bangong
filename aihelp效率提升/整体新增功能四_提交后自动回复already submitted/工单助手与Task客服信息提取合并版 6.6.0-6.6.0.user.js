// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.6.0
// @namespace    http://tampermonkey.net/
// @version      6.6.0
// @description  新增飞书项目Ticket ID搜索功能：工单ID变化时自动提取Ticket ID并在飞书项目后台搜索，结果展示在日志面板
...
/**
 * v6.6.0 (2026-03-18) 新增飞书项目 Ticket ID 搜索功能
 *
 * 【新增功能 - 模块E：飞书 Ticket ID 搜索】
 * 需求来源：提示词总结和skills2026-2-13\通用提示词模版.md
 * 功能说明：
 *   - 每次工单ID变化时，从工单内部描述中提取 Ticket ID 的值
 *     （格式如 "Ticket ID= Y372U8"，提取结果为 "Y372U8"）
 *   - 在后台标签页打开飞书项目网页，在搜索框中搜索该 Ticket ID
 *     目标网页：https://project.feishu.cn/ml/workObjectView/onlineissue/Cot68m5vg?scope=workspaces&node=28514456
 *   - 搜索结果同步到日志面板：
 *     - 若找到结果：红色显示"在项目中搜索到该 Ticket ID"
 *     - 若未找到：普通提示"未搜到该 Ticket ID"
 *   - 后台标签页静默运行，不打扰当前 AIHelp 页面操作
 *   - 若用户未登录飞书，在日志面板提示用户登录
 * 实现机制：
 *   - AIHelp端（模块E-AIHelp）：提取Ticket ID → GM_setValue写入 → window.open后台标签页
 *   - 飞书端（模块E-飞书）：读取待搜索ID → 操控搜索框 → 检测结果 → GM_setValue回写
 *   - AIHelp端轮询 GM_getValue 获取搜索结果并更新日志面板
 * 耦合性：模块独立封装，互不干扰现有功能
 *
 * 【历史更新】
...
(function() {
    ...
    (function() {
        ...
        function triggerFeishuSearch(ticketId) {
            ...
            //    需求：不要打扰用户在当前页面的操作
            try {
                // 如果之前的标签页还存在，直接复用（reload触发飞书端重新检测）
                if (feishuSearchState.feishuTabRef && !feishuSearchState.feishuTabRef.closed) {
                    feishuSearchState.feishuTabRef.location.href = FEISHU_SEARCH_CONFIG.feishuTargetUrl;
                    feishuLog('复用已有飞书标签页，触发刷新');
                } else {
                    feishuSearchState.feishuTabRef = window.open(
                        FEISHU_SEARCH_CONFIG.feishuTargetUrl,
                        'feishu_ticket_search_tab'  // 指定名称，防止重复开多个标签
                    );
                    feishuLog('已打开飞书后台标签页');
                }
            } catch (e) {
    ...
    // =========================================================================
    // 模块 E-飞书端：飞书项目 Ticket ID 搜索执行器（完全独立）
    //
    // 需求来源：提示词总结和skills2026-2-13\通用提示词模版.md
    // 功能：
    //   - 仅在飞书项目目标页面运行
    //   - 读取 GM_getValue 中的待搜索 Ticket ID
    //   - 在搜索框（input.semi-input[placeholder="按标题查找"]）中输入 Ticket ID
    //   - 等待搜索结果列表更新，检测是否有结果
    ...
    (function() {
        ...
        function isFeishuLoggedIn() {
            const bodyText = document.body.innerText || '';
            // 飞书未登录时通常显示登录引导文本
            if (bodyText.includes('登录') && bodyText.includes('账号') && bodyText.length < 500) {
                return false;
            }
            // 检查是否有飞书项目的关键 DOM 元素（搜索框容器）
            const searchContainer = document.getElementById('story-view-search-container');
        ...
        function detectSearchResult() {
            return new Promise((resolve) => {
                ...
                function checkResult() {
                    // 飞书工作项列表：通常使用 tr 或特定的工作项容器
                    // 搜索无结果时，飞书会显示空状态提示（通常包含"暂无数据"或"没有找到"等文字）
                    const bodyText = document.body.innerText || '';

                    // 检查空结果标记（飞书常见的空状态文本）
                    const emptyTexts = ['暂无数据', '没有找到', '无搜索结果', 'No data', '暂无内容'];
                    const hasEmptyState = emptyTexts.some(t => bodyText.includes(t));

                    // 检查是否有工作项列表行（tr 或工作项容器出现说明有结果）
                    // 飞书工作项行通常带有特定 class
                    const listRows = document.querySelectorAll('tr.story-list-table-row, [class*="story-row"], [class*="work-item-row"]');
        ...
        async function executeFeishuSearch(ticketId, requestTs) {
            ...
            try {
                ...
                // 2. 等待搜索框出现（飞书重型应用需要等待 DOM 就绪）
                feishuExecLog('等待搜索框就绪...');
                // 搜索框容器标识：id="story-view-search-container"
                // 内部 input：placeholder="按标题查找"
                const searchInput = await waitForElement(
                    'input.semi-input.semi-input-default[placeholder="按标题查找"]',
                    FEISHU_EXEC_CONFIG.searchResultTimeout
                );

                if (!searchInput) {
                    feishuExecLog('未找到搜索框，尝试备用选择器...');
                    // 备用：通过容器 ID 定位
                    const container = document.getElementById('story-view-search-container');
                    if (!container) {
                        feishuExecLogError('搜索框容器未找到，可能未登录或页面结构变化');
                        // 二次登录状态检测
                        if (!isFeishuLoggedIn()) {
                            writeSearchResult('not_logged_in');
                        } else {
                            writeSearchResult('error', '搜索框未找到，页面结构可能已变化');
                        }
                        return;
                    }
                    const fallbackInput = container.querySelector('input[type="text"]');
                    if (!fallbackInput) {
                        writeSearchResult('error', '搜索框内 input 未找到');
                        return;
                    }
                    feishuExecLog('使用备用搜索框');
                    // 继续使用 fallbackInput
                    const inputResult = simulateSearchInput(fallbackInput, ticketId);
                    if (!inputResult) {
                    ...
                    // 3. 向搜索框输入 Ticket ID
                    feishuExecLog('找到搜索框，开始输入 Ticket ID:', ticketId);
                ...
                // 4. 等待搜索结果更新（飞书搜索通常有 debounce，稍等一下）
        ...
        function initFeishuExecModule() {
            feishuExecLog('========================================');
            feishuExecLog('飞书 Ticket ID 搜索执行器 v6.6.0 已启动');
