// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.7.0
// @namespace    http://tampermonkey.net/
// @version      6.7.0
// @description  新增清除头像冷却时间功能：在工单页面提取UID和ServerID，自动打开GM工具执行清除操作，日志同步到主面板
    ...
    class StatusbarUI {
        ...
        injectStyles() {
            GM_addStyle(`
                ...
                /* [模块F] 区域6：展开面板（底部右格，保留⚡样式） */
                .ai-icon-zone-expand-bottom {
                    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                    color: white;
                    border-radius: 0 0 6px 0;
                }
        ...
        createDOM() {
            ...
            // 组装图标容器（6格 2列×3行：N/M/T/⚡/清/空白占位）
            // 第1行：N（普通）、M（MCGG）
            // 第2行：T（Task）、⚡（展开）
            // 第3行：清（清除头像）、[空白占位]
            const zonePlaceholder = document.createElement('div');
            zonePlaceholder.className = 'ai-icon-zone';
            zonePlaceholder.style.cssText = 'background: #f5f5f7; border-radius: 0 0 6px 0; cursor: default;';

            this.iconElement.append(zoneNormal, zoneMcgg, zoneTask, zoneExpand, zoneClear, zonePlaceholder);
            this.zones = { normal: zoneNormal, mcgg: zoneMcgg, task: zoneTask, expand: zoneExpand, clear: zoneClear };
