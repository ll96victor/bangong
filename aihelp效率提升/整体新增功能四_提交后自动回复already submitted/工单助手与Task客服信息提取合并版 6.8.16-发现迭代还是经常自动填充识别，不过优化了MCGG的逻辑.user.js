// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.8.16
...
(function() {
    ...
    const SharedUtils = {
        ...
        isIterationFieldValueApplied(labelTexts = [], expectedValue = '') {
            const currentValue = this.getThirdLinkFieldDisplayValue(labelTexts);
        ...
        waitForIterationFieldValue(labelTexts = [], expectedValue = '', timeout = 1500) {
            return new Promise(resolve => {
                const startTime = Date.now();
                const check = () => {
                    if (this.isIterationFieldValueApplied(labelTexts, expectedValue)) {
                        resolve(true);
                        return;
                    }
    ...
    (function() {
        ...
        async function handleIterationFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearchLegacy(state.faxiandiedai, logger);
        ...
        async function autoFillThirdLinkField(labelTexts = [], value = '', stateKey = '', fieldName = '', options = {}) {
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                if (!opened) {
                    ...
                    const success = isIterationField
                        ? await SharedUtils.fillDropdownSearchLegacy(value, logger, 150, { preferThirdLink: true })
                        : await SharedUtils.fillDropdownSearch(value, logger, 150, { preferThirdLink: true });

                    if (isIterationField) {
                        if (success) {
                            const applied = await SharedUtils.waitForIterationFieldValue(labelTexts, value, CONFIG.thirdLinkConfirmTimeout);
                            if (applied) {
                                state[stateKey] = true;
                        ...
                        const applied = await SharedUtils.confirmThirdLinkFieldApplied(labelTexts, value, CONFIG.thirdLinkConfirmTimeout, options);
                        if (applied) {
                            state[stateKey] = true;
                            if (!success) {
            ...
            const failMsg = fieldName + ' \u81ea\u52a8\u586b\u5145\u5931\u8d25\uff0c\u4fdd\u7559\u7126\u70b9\u76d1\u542c\u515c\u5e95';
        ...
        async function processThirdLinkFields(options = {}) {
            const { skipServerDependentFields = false } = options;
        ...
        async function processTicket() {
            ...
            try {
                ...
                await processThirdLinkFields({ skipServerDependentFields: !hasValidServer });
                setupFocusListener();
                log('========== 普通工单处理完成 ==========');
            } catch (e) {
    ...
    (function() {
        ...
        async function processMCGGTicket() {
            ...
            try {
                ...
                await processMCGGThirdLinkFields({ skipServerDependentFields: !hasValidServer });
