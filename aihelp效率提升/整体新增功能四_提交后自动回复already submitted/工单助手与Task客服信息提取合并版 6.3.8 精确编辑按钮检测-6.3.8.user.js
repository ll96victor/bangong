// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.3.8 精确编辑按钮检测
// @namespace    http://tampermonkey.net/
// @version      6.3.8
// @description  精确匹配"编辑"按钮 批量处理时存在"编辑"或"编辑筛选项"按钮，检测到则跳过自动回复
 ...
 *
 * 【修复问题】
 * 1. 修复"编辑"按钮检测逻辑，改为精确匹配
 *    - 之前使用 includes('编辑') 会误匹配到其他包含"编辑"的元素
 *    - 现在使用精确匹配 === '编辑'，只匹配按钮文本完全等于"编辑"的情况
 *
 * 2. 新增"编辑筛选项"按钮检测逻辑
 *    - 检测到"编辑筛选项"按钮存在时，跳过自动回复
 *
 * 3. 恢复批量处理模式检测逻辑
 *    - 检测"编辑"按钮是否存在，存在则跳过自动回复
 *    - 检测"编辑筛选项"按钮是否存在，存在则跳过自动回复
 *    - 检测当前工单ID，无ID则跳过
 *    - 检测"内部回复"按钮是否存在，不存在则跳过
 *
 * 4. 修复自动回复文本框查找逻辑，添加重试机制
 *    - 最多尝试10次查找TinyMCE编辑器
 *    - 每次间隔300ms，确保编辑器有足够时间加载
 *
 * 【保留功能】
        ...
        function setupConfirmButtonObserver() {
            document.addEventListener('click', function(e) {
                ...
                if (clickableEl) {
                    ...
                    const editButtons = document.querySelectorAll('span, button');
                    for (const btn of editButtons) {
                        const btnText = (btn.textContent || '').trim();
                        const normalizedBtnText = btnText.replace(/\s+/g, '');

                        if (normalizedBtnText === '编辑') {
                            const style = window.getComputedStyle(btn);
                            if (style.display !== 'none' && style.visibility !== 'hidden') {
                                autoReplyLog('检测到"编辑"按钮存在，跳过自动回复（批量处理模式）');
                                autoReplyLog('[DEBUG] 按钮元素:', btn.tagName, '| 类名:', btn.className, '| 父元素:', btn.parentElement?.tagName, '| 父类名:', btn.parentElement?.className);
                                return;
                            }
                        }
                        if (btnText.includes('编辑筛选项')) {
                            const style = window.getComputedStyle(btn);
                            if (style.display !== 'none' && style.visibility !== 'hidden') {
                                autoReplyLog('检测到"编辑筛选项"按钮存在，跳过自动回复（批量处理模式）');
                                return;
                            }
                        }
                    }
