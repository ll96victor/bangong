...
(function() {
    ...
    const SharedUtils = {
        ...
        waitForDropdownSearchInput(timeout = 1200) {
            return new Promise(resolve => {
                ...
                const check = () => {
                    const dropdown = this.findVisibleDropdown();
                    if (dropdown) {
        ...
        async selectDropdownOption(text, logger, timeout = 1200) {
            const startTime = Date.now();
            ...
            while (Date.now() - startTime < timeout) {
                ...
                if (dropdown) {
                    ...
                    if (items.length > 0) {
                        ...
                        const selectedMatch = items.find(item => item.classList.contains('selected'));
                        const candidate = exactMatch || partialMatch || selectedMatch || items[0];
        ...
        async fillDropdownSearch(text, logger, delay = 100) {
            ...
            try {
                ...
                await new Promise(resolve => setTimeout(resolve, delay));
                const selected = await this.selectDropdownOption(text, logger);
    ...
    (function() {
        ...
        function findThirdLinkFieldRow(labelTexts = []) {
            const normalizedTargets = labelTexts.map(label => SharedUtils.normalizeFieldLabel(label));
            ...
            for (const row of rows) {
                ...
                const titleText = SharedUtils.normalizeFieldLabel(titleNode ? titleNode.textContent : '');
                if (titleText && normalizedTargets.includes(titleText)) {
                    return row;
                }
            }
            return null;
        ...
        function findThirdLinkFieldInput(labelTexts = []) {
            const row = findThirdLinkFieldRow(labelTexts);
            if (!row) return null;
            return row.querySelector('.detail input.el-input__inner, .detail input[type="text"], .detail input');
        }

        async function openThirdLinkDropdown(labelTexts = [], fieldName = '') {
            const maxAttempts = 4;

            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                const input = findThirdLinkFieldInput(labelTexts);
                if (!input) {
                    await new Promise(resolve => setTimeout(resolve, 300));
                    continue;
                }

                const clickable = input.closest('.el-select') || input.closest('.el-input') || input;
                ...
                const dropdownInput = await SharedUtils.waitForDropdownSearchInput(1500);
                if (dropdownInput) {
        ...
        function extractInternalDescription() {
            ...
            if (outerHtmlExtracted) {
                ...
                log('通过 outerHTML 提取内部描述成功，长度:', outerHtmlExtracted.length);
        ...
        async function autoFillThirdLinkField(labelTexts = [], value = '', stateKey = '', fieldName = '') {
            ...
            const opened = await openThirdLinkDropdown(labelTexts, fieldName);
            if (!opened) {
    ...
    (function() {
        ...
        const FEISHU_SEARCH_CONFIG = {
            ...
            storageKeyHeartbeat: 'feishu_ticket_search_heartbeat_v1', // 飞书目标页心跳，避免重复打开标签页
        ...
        function hasFreshFeishuOpenLock() {
            ...
            return (Date.now() - lockData.ts) < 20000;
        }
        ...
        function isFeishuTargetPageAlive() {
            try {
                ...
                feishuLogError('读取飞书目标页心跳失败:', e.message);
                return false;
            }
        }
