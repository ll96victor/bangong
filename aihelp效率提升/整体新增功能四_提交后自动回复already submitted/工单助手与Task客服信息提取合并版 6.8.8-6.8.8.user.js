...
(function() {
        ...
        function triggerFeishuSearch(ticketId) {
            ...
            try {
                if (isFeishuTargetPageAlive()) {
                    feishuLog(feishuSearchFmtReq(searchRequestId, '检测到飞书目标页心跳有效，复用现有目标页执行搜索'));
                } else if (hasFreshFeishuOpenLock()) {
                    feishuLog(feishuSearchFmtReq(searchRequestId, '检测到飞书目标页正在拉起中，复用本次打开动作'));
                } else {
                    ...
                    feishuLog(feishuSearchFmtReq(searchRequestId, '未检测到有效目标页心跳，已尝试唤起飞书目标页'));
                }
            } catch (e) {
                clearFeishuOpenLock();
    ...
    (function() {
        ...
        async function clickMoreButton() {
            const btn = document.querySelector('.new-top button.el-button.more') ||
                document.querySelector('button.el-button.more');
            if (!btn) {
        ...
        async function clickSendRewardMenuItem() {
            const items = Array.from(document.querySelectorAll('.el-dropdown-menu li.el-dropdown-menu__item'));
            const target = items.find(li => (li.textContent || '').replace(/\s+/g, '').includes('发送奖励'));
            if (!target) {
