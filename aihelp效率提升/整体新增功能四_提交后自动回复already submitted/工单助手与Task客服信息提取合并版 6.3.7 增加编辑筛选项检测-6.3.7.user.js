// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.3.7 增加编辑筛选项检测
// @namespace    http://tampermonkey.net/
// @version      6.3.7
// @description  新增"编辑"和"编辑筛选项"按钮检测 批量处理时存在这些按钮，检测到则跳过自动回复
...
/**
 * 6.3.7 增加编辑筛选项检测 更新说明：
 *
 * 【修复问题】
 * 1. 新增"编辑筛选项"按钮检测逻辑
 *    - 检测到"编辑筛选项"按钮存在时，跳过自动回复
 *    - 与"编辑"按钮检测逻辑一致，只是按钮名称不同
 *
 * 2. 恢复批量处理模式检测逻辑，修复批量处理时自动回复误触发问题
 *    - 检测"编辑"按钮是否存在，存在则跳过自动回复
 *    - 检测"编辑筛选项"按钮是否存在，存在则跳过自动回复
 *    - 检测当前工单ID，无ID则跳过
 *    - 检测"内部回复"按钮是否存在，不存在则跳过
 *
 * 3. 修复自动回复文本框查找逻辑，添加重试机制
 *    - 最多尝试10次查找TinyMCE编辑器
            ...
            document.addEventListener('click', function(e) {
                ...
                if (clickableEl) {
                    ...
                    for (const btn of editButtons) {
                        const btnText = (btn.textContent || '').trim();
                        if (btnText.includes('编辑') && !btnText.includes('内部描述') && !btnText.includes('任务标题')) {
