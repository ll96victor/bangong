// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.4.7
// @namespace    http://tampermonkey.net/
// @version      6.4.7
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：修复DeepLX公开镜像，预留GLM大模型翻译接口，精简失效接口，支持通过菜单配置API Key
 ...
 * 【安全与配置】
 * 1. 移除硬编码 API Key：
 *    - 不再将 Key 直接写在代码中，避免分享脚本时泄露
 * 2. 新增油猴菜单配置功能：
 *    - 用户可通过浏览器插件菜单 "设置智谱 GLM-4-Flash API Key" 随时填入或修改 Key
 *    - Key 安全存储在本地油猴扩展中（GM_setValue/GM_getValue）
 * 3. 优化 GLM 调用逻辑：
 *    - 如果未配置 Key，脚本会自动跳过 GLM 翻译，不会报错或弹窗打扰
 *    - 只有在配置了 Key 后才会启用大模型翻译功能
 *
 * 【历史更新】
 * v6.4.6: 修复 DeepLX 接口，预留 GLM 接口
 * v6.4.5: 优化日志翻译面板，支持失败状态显示与对比
 */
...
(function() {
    ...
    (function() {
        ...
        function translateViaGoogle(text) {
            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    ...
                    timeout: CONFIG.translateTimeoutGoogle,
                    onload: (response) => {
                        try {
        ...
        function translateViaDeepLX_Mirror(text) {
            return new Promise((resolve, reject) => {
                // 使用国内免部署的公开 DeepLX 镜像
                GM_xmlhttpRequest({
                    method: 'POST',
                    url: 'https://api.deeplx.fun/translate',
                    ...
                    onload: (response) => {
                        try {
                            ...
                            if (result.code === 200 && result.data) {
                                ...
                                reject(new Error('DeepLX(Mirror) error'));
                            }
                        } catch (e) {
                            reject(e);
                        }
        ...
        function translateViaGLM4Flash(text) {
            return new Promise((resolve, reject) => {
                // 使用 GM_getValue 获取存储的 Key，如果没有则尝试请求用户输入
                let apiKey = GM_getValue('glm_api_key_v1', '');

                if (!apiKey) {
                    // 仅提示一次，如果用户取消则本次不尝试 GLM 翻译
                    // 注意：在并发 Promise.all 中，prompt 可能会阻塞其他请求，但 JS 单线程特性意味着 alert/prompt 会暂停执行
                    // 这里为了不打扰用户频繁弹出，建议只在用户明确想要用 GLM 时才弹，或者在菜单里设置
                    // 现在的策略是：如果没 Key，直接 reject，不弹窗，避免自动运行时打断用户
                    // 用户可以通过油猴菜单手动设置 Key
                ...
                GM_xmlhttpRequest({
                    ...
                    data: JSON.stringify({
                        model: "glm-4-flash",
                        messages: [
                            { role: "system", content: "你是一个专业的翻译助手。请将以下内容直接翻译为中文，不要输出任何额外的解释或说明文本。" },
        ...
        async function translateText(text) {
            ...
            const translators = [
                ...
                { name: 'DeepLX(Mirror)', fn: translateViaDeepLX_Mirror, timeout: CONFIG.translateTimeoutOther },
                { name: 'Popcat', fn: translateViaPopcat, timeout: CONFIG.translateTimeoutOther },
                { name: 'MyMemory', fn: translateViaMyMemory, timeout: CONFIG.translateTimeoutOther },
                { name: 'GLM-4-Flash', fn: translateViaGLM4Flash, timeout: CONFIG.translateTimeoutOther }
            ];
            ...
            Promise.all(promises).then(allResults => {
                // 只要有结果返回（无论成功失败），我们都可能想展示面板，特别是为了显示失败原因
                // 但如果所有都失败了，且没有成功结果，面板显示也没意义，不如只显示日志
        ...
        function initUI() {
            ...
            GM_registerMenuCommand('设置智谱 GLM-4-Flash API Key', () => {
                const currentKey = GM_getValue('glm_api_key_v1', '');
                const newKey = prompt('请输入智谱 GLM-4-Flash API Key (永久免费，获取方式见智谱官网):', currentKey);
                if (newKey !== null) {
