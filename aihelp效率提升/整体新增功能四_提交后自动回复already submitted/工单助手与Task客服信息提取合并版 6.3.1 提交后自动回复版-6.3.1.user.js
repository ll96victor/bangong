// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.3.1 提交后自动回复版
// @namespace    http://tampermonkey.net/
// @version      6.3.1
// @description  提交后自动回复版：点击确定后自动回复"already submitted" + 日志面板可拖拽调整 + 位置大小记忆 + 四模块独立架构
 ...
 *
 * 【新增功能：提交后自动回复】
 * 1. 自动监听"确定"按钮状态变化（使用MutationObserver）
 * 2. 检测到按钮从加载状态恢复后，自动触发回复流程
 * 3. 自动点击"内部回复"按钮，打开回复对话框
 * 4. 自动填充固定文本 "already submitted"
 * 5. 支持普通工单和MCGG工单两种页面类型
 *
 * 【延迟与超时控制】
 * 6. 可配置的对话框加载等待时间（1-2秒）
 * 7. 5秒超时机制：元素查找超时后静默退出
 * 8. 防抖机制：防止重复触发
 *
 * 【架构设计】
 * 9. 完全独立的IIFE模块，不与现有功能冲突
 * 10. 使用独立的配置对象和状态管理
 * 11. 集成到日志面板，可通过日志查看执行状态
 *
 * 【保留原有功能】
 * - 日志面板可拖拽调整大小
 * - 位置和大小记忆功能
 * - 四模块完全独立：普通工单、MCGG、Task、自动回复
 * - 配置完全分离，状态完全隔离
 */
    ...
    (function() {
        ...
        function setupConfirmButtonObserver() {
            document.addEventListener('click', function(e) {
                const target = e.target;
                const clickableEl = target.closest('button, .el-button, [role="button"], span.el-button, a, [class*="btn"]');
                ...
                if (clickableEl) {
                    ...
                    if (normalizedText === '确认' || normalizedText === '确定' || elText.includes('确认') || elText.includes('确定')) {
