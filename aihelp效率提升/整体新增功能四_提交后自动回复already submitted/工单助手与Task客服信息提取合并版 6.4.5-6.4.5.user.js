// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.4.5
// @namespace    http://tampermonkey.net/
// @version      6.4.5
// @description  增加批量筛选模式检测，支持展开面板压缩，升级翻译功能：引入Microsoft等更多免费源，日志面板支持失败状态显示与对比
...
/**
 * 6.4.5 翻译源增强与面板优化 更新说明：
 *
 * 【新增功能】
 * 1. 新增 Microsoft Edge 免费翻译接口：
 *    - 质量极高，作为强力备用源
 *    - 无需 API Key，使用 Edge 浏览器公开接口
 * 2. 优化日志翻译面板：
 *    - 现在会显示所有翻译源的状态（包括失败的）
 *    - 失败的源会显示红色错误信息（如 timeout 或 network error）
 *    - 方便用户直观对比不同源的结果，了解为什么某些源没有结果
 *
 * 【技术说明】
 * - 关于百度/腾讯/阿里/火山/小牛翻译：
 *   这些国内服务均强制验证 API Key 和复杂的请求签名，无法在纯油猴脚本中稳定免 Key 调用。
 *   建议使用本次新增的 Microsoft 翻译作为替代，其质量与国内大厂相当。
 *
 * 【历史更新】
...
(function() {
    ...
    (function() {
        ...
        function translateViaDeepLX(text) {
            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    method: 'POST',
        ...
        function translateViaYandex(text) {
             // Yandex usually requires a key or scraping, skipping for now as it's unstable without key
             // Placeholder for future implementation if a stable no-key method is found
             return Promise.reject(new Error('Yandex requires API Key'));
        }
        ...
        async function translateText(text) {
            ...
            const translators = [
                ...
                { name: 'Microsoft', fn: translateViaMicrosoft, timeout: CONFIG.translateTimeoutOther },
                { name: 'DeepLX(AI)', fn: translateViaDeepLX, timeout: CONFIG.translateTimeoutOther },
