// ==UserScript==
// @name         工单助手与Task客服信息提取合并版 6.8.18
...
(function() {
    ...
    (function() {
        ...
        async function handleIterationFocus() {
            ...
            const success = await SharedUtils.fillDropdownSearch(state.faxiandiedai, logger, 150, { preferThirdLink: true });
        ...
        async function autoFillThirdLinkField(labelTexts = [], value = '', stateKey = '', fieldName = '', options = {}) {
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                if (!opened) {
                    ...
                    const applied = await SharedUtils.confirmThirdLinkFieldApplied(labelTexts, value, confirmTimeout, options);
                    if (applied) {
                        state[stateKey] = true;
                        if (!success) {
