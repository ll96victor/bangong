...
(function() {
    ...
    function searchAtKeyword() {
        if (moduleStates.searchAt.isProcessing) {
            showToast('操作过于频繁，请稍候...');
            return;
        }
        ...
        setTimeout(() => {
            let foundNode = null;
            let foundKeyword = null;
            let foundRegexMatch = null;

            // 用于匹配 "于 2026-03-13 11:51:49 评论" 这类格式的正则
            ...
            while (node = walker.nextNode()) {
                allNodes.push(node);
                if (node.textContent.includes('@') && isElementVisible(node.parentElement)) {
                    foundNode = node;
                    foundKeyword = '@';
                    break;
                }
            ...
            if (foundNode) {
                let fragment;
                if (foundKeyword) {
                    fragment = highlightTextNode(foundNode, foundKeyword);
                } else if (foundRegexMatch) {
                    fragment = highlightTextNode(foundNode, foundRegexMatch);
                }
                ...
                showToast('✅');
                if (foundKeyword) {
                    atLogger.success('找到 @ 标记');
                } else {
                    atLogger.success('找到非 Albin 的评论记录');
                }
