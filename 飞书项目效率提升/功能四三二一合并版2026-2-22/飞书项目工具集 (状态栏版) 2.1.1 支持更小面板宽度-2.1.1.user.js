// ==UserScript==
// @name         飞书项目工具集 (状态栏版) 2.1.1 支持更小面板宽度
// @namespace    http://tampermonkey.net/
// @version      2.1.1
// @description  合并多个飞书项目相关工具脚本，使用状态栏UI设计。新增自动评论功能（已联系/未回复）。日志面板最小宽度支持80px。
...
(function() {
    ...
    function searchKeywords() {
        ...
        setTimeout(() => {
            ...
            if (foundNode && foundKeyword) {
                ...
                showToast('✅');
                searchLogger.success('关键词检索成功: ' + foundKeyword);
            } else {
                showToast('❌ 未联系');
    ...
    async function handleAutoComment(commentType) {
        ...
        try {
            ...
            if (publishSuccess) {
                ...
                autoCommentLogger.warn('未找到发布按钮');
            }
        } catch (error) {
    ...
    function createLogPanel() {
        ...
        panel.innerHTML = `
            ...
            <div id="ai-log-panel-actions">
                <button class="ai-action-btn" data-action="search">🔍 已联系</button>
                <button class="ai-action-btn" data-action="extract-info">📋 信息提取</button>
                <button class="ai-action-btn" data-action="extract-links">⚡ 链接提取</button>
                <button class="ai-action-btn" data-action="smart-links">🔗 打开链接</button>
                <button class="ai-action-btn" data-action="contacted">已 已联系</button>
                <button class="ai-action-btn" data-action="no-reply">未 未回复</button>
                <button class="ai-action-btn" data-action="clear-logs" style="color: #86868b;">🗑️ 清空日志</button>
            </div>
    ...
    function init() {
        ...
        addLogToPanel('🔗 打开链接 - 提取并打开aihelp链接', 'info', 'smart');
