// ==UserScript==
// @name         平滑滚动导航器 v3.2.2（多滚动容器感知 + Canvas表格兼容）
...
(function () {
  ...
  // ─── 注入 DOM（兼容 SPA 延迟加载和路由切换） ───────────────────────
  function injectNav() {
    if (document.getElementById('scroll-navigator')) return;
  ...
  function ensureNav() {
    injectNav();
    // SPA 路由切换可能导致 body 子树被替换，持续监听并重新注入
    const observer = new MutationObserver(() => {
      if (!document.getElementById('scroll-navigator')) injectNav();
    });
    const watchTarget = document.body || document.documentElement;
    if (watchTarget) observer.observe(watchTarget, { childList: true, subtree: false });
  }

  if (document.body) {
    ensureNav();
  } else {
    // 极端情况：document-idle 之前 body 仍不存在
    document.addEventListener('DOMContentLoaded', ensureNav);
    // 再加一层保险
    setTimeout(ensureNav, 2000);
  }
