...
(function() {
    ...
    function extractFeishuOrder() {
        try {
            const noteItem = document.querySelector('.note-item, .chat-note-wrap');
            if (!noteItem) return null;

            const feishuLink = noteItem.querySelector('a[href*="project.feishu.cn"]');
            if (!feishuLink) return null;

            const linkText = feishuLink.textContent.trim();
            const linkHref = feishuLink.getAttribute('href');

            const timeSpan = noteItem.querySelector('.note-time');
            let timeText = '';
            if (timeSpan) {
