...
// @author       AI Assistant
// @match        https://ml-panel.aihelp.net/dashboard/#/manual/tasks*
...
(function() {
    ...
    function isTicketPage() {
        ...
        return url.includes('ml-panel.aihelp.net') && url.includes('tasks');
    }
    ...
    async function executeClearAvatar() {
        ...
        // 从localStorage获取待处理的任务数据
        const taskData = loadFromStorage(STORAGE_KEYS.PENDING_TASK);
        if (!taskData) {
            log('没有待处理的任务', 'warn');
            return;
        }
        ...
        // 检查是否在正确的页面
        if (!window.location.href.includes('banTool')) {
            log('当前不在banTool页面，尝试跳转...', 'warn');
            window.location.href = CONFIG.GM_TOOL_URL;
            return;
        }
        ...
        try {
            ...
            const serverIdBox = await waitForAnyElement(serverIdSelectors, 5000);
            if (!serverIdBox) {
                log('未找到ServerID选择框', 'error');
                return;
            }
            ...
            // 步骤3：输入ServerID并选择匹配项
            log('输入ServerID: ' + serverId);
            
            // 查找下拉框中的输入框
            const searchInput = await waitForElement('.ant-select-search-input, .ant-select-input input, input[type="text"]', 3000);
            if (searchInput) {
                ...
                // 选择第一个匹配项
                const firstOption = await waitForElement('.ant-select-item-option, .ant-select-dropdown-menu-item', 2000);
                if (firstOption) {
                    log('选择匹配的ServerID选项...');
                    firstOption.click();
                    await sleep(500);
                } else {
                    log('未找到匹配的ServerID选项', 'warn');
                }
            } else {
                log('未找到搜索输入框', 'error');
                return;
            ...
            // 步骤4：滚动到"清除上传头像倒计时"区域
            log('查找"清除上传头像倒计时"区域...');
            
            // 使用多种方式查找
            const allTitles = document.querySelectorAll('.title, [class*="title"]');
            let targetTitle = null;
            
            for (const title of allTitles) {
                if (title.textContent.includes('清除上传头像倒计时')) {
                    targetTitle = title;
                    break;
                }
            }
            ...
            if (targetTitle) {
                ...
                log('未找到"清除上传头像倒计时"区域，尝试滚动页面...', 'warn');
                // 滚动到页面中部
            ...
            // 在"清除上传头像倒计时"区域附近查找输入框
            let uidInput = null;
            
            // 先尝试在目标区域附近查找
            if (targetTitle) {
                const parent = targetTitle.closest('[class*="item"], [class*="row"], div');
                if (parent) {
                    for (const selector of uidInputSelectors) {
                        uidInput = parent.querySelector(selector);
                        if (uidInput) break;
                    }
                }
            }
            ...
            // 如果没找到，全局查找
            if (!uidInput) {
                const allInputs = document.querySelectorAll('input[type="text"], input.el-input__inner');
                for (const input of allInputs) {
                    const placeholder = input.getAttribute('placeholder') || '';
                    if (placeholder.toUpperCase().includes('UID')) {
                        uidInput = input;
                        break;
                    }
                }
            ...
            // 如果还是没找到，尝试查找所有输入框
            if (!uidInput) {
                uidInput = await waitForAnyElement(uidInputSelectors, 3000);
            }
            ...
            // 查找播放图标按钮（执行按钮）
            const playButtons = document.querySelectorAll('.el-icon-video-play, i[title="运行"], [class*="play"]');
            let executeBtn = null;
            
            for (const btn of playButtons) {
                // 确保按钮在"清除上传头像倒计时"区域附近
                const parent = btn.closest('[class*="item"], [class*="row"]');
                if (parent && parent.textContent.includes('清除上传头像倒计时')) {
                    executeBtn = btn;
                    break;
                }
            }
            
            // 如果没找到，尝试查找包含"执行"或"运行"的按钮
            if (!executeBtn) {
                const allButtons = document.querySelectorAll('button, .el-button, i[class*="icon"]');
                for (const btn of allButtons) {
                    const title = btn.getAttribute('title') || '';
                    const text = btn.textContent || '';
                    if (title.includes('运行') || text.includes('执行')) {
                        executeBtn = btn;
                        break;
                    }
                }
            ...
            } else if (resultText.includes('Result:Error') || resultText.includes('Result: Error')) {
                log('清除头像冷却时间失败，请检查参数', 'error');
            } else {
                log('操作已执行，请检查结果', 'warn');
            }
            
            // 清除待处理任务
            deleteFromStorage(STORAGE_KEYS.PENDING_TASK);
            log('任务完成，已清除待处理数据');
    ...
    function main() {
        ...
        if (isTicketPage()) {
            ...
            // 创建日志通道
            const logger = UI.createLogChannel();
            window.clearAvatarLogger = (msg, type) => logger[type === 'info' ? 'log' : type](msg);
            
            // 注册"清除头像"区域点击回调
            UI.registerZoneCallback('clear', async (element) => {
                log('点击了清除头像区域');
                ...
                try {
                    ...
                    // 保存任务数据到localStorage
                    saveToStorage(STORAGE_KEYS.PENDING_TASK, accountInfo);
            ...
            if (taskData) {
                ...
                if (document.readyState === 'complete') {
                    setTimeout(executeClearAvatar, 1000);
                } else {
