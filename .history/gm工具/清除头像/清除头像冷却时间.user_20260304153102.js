    ...
    function initTicketPage() {
        ...
        uiInstance.registerZoneCallback('clear', async (element) => {
            ...
            try {
                ...
                // 方案二：iframe嵌入方式（已注释，跨域限制不方便）
                // log('正在创建GM工具iframe...');
                // let existingIframe = document.getElementById('ca-gm-tool-iframe');
                // if (existingIframe) { existingIframe.remove(); }
                // const overlay = document.createElement('div');
                // overlay.id = 'ca-gm-tool-overlay';
                // overlay.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 999998; display: flex; align-items: center; justify-content: center;';
                // const iframeContainer = document.createElement('div');
                // iframeContainer.style.cssText = 'position: relative; width: 90%; height: 90%; background: #fff; border-radius: 12px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); overflow: hidden;';
                // const closeBtn = document.createElement('button');
                // closeBtn.textContent = '×';
                // closeBtn.style.cssText = 'position: absolute; top: 10px; right: 10px; width: 30px; height: 30px; border: none; background: #ff4d4f; color: white; font-size: 20px; border-radius: 50%; cursor: pointer; z-index: 10;';
                // closeBtn.onclick = () => { overlay.remove(); log('iframe已关闭'); };
                // const iframe = document.createElement('iframe');
                // iframe.id = 'ca-gm-tool-iframe';
                // iframe.src = CONFIG.GM_TOOL_URL;
                // iframe.style.cssText = 'width: 100%; height: 100%; border: none;';
                // iframeContainer.appendChild(closeBtn);
                // iframeContainer.appendChild(iframe);
                // overlay.appendChild(iframeContainer);
                // document.body.appendChild(overlay);
                // log('iframe已创建', 'success');
