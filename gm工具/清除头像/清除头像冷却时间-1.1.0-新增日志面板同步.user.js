...
(function() {
    ...
    const STORAGE_KEYS = {
        ...
        // 弹窗模式标记
        POPUP_MODE: CONFIG.STORAGE_PREFIX + 'popup_mode',

        // 跨域日志数据（GM工具页面同步到工单页面）
        LOG_DATA: CONFIG.STORAGE_PREFIX + 'log_data',
    ...
    class StatusbarUI {
        ...
        constructor(config = {}) {
            ...
            // 跨域日志轮询相关
            this.logPollingTimer = null;        // 轮询定时器
            this.lastLogTimestamp = 0;          // 已显示的最后一条日志时间戳
            this.logPollingInterval = 500;      // 轮询间隔（毫秒）
        ...
        startLogPolling() {
            ...
            // 立即执行一次
            this.pollCrossDomainLogs();

            // 设置定时轮询
            this.logPollingTimer = setInterval(() => {
                this.pollCrossDomainLogs();
            }, this.logPollingInterval);
        }
        ...
        /**
         * 轮询读取跨域日志数据
         * 只显示比 lastLogTimestamp 更新的日志
         */
        pollCrossDomainLogs() {
            try {
                // 从跨域存储读取日志数据
                const logData = loadCrossDomainData(STORAGE_KEYS.LOG_DATA, []);

                if (!Array.isArray(logData) || logData.length === 0) {
                    return;
                }

                // 筛选出新日志（时间戳大于 lastLogTimestamp）
                const newLogs = logData.filter(log => {
                    return log.timestamp && log.timestamp > this.lastLogTimestamp;
                });
                ...
                for (const logEntry of newLogs) {
                    this.addLogFromCrossDomain(logEntry);
                }

                // 更新最后日志时间戳
                const lastLog = newLogs[newLogs.length - 1];
                this.lastLogTimestamp = lastLog.timestamp;

            } catch (e) {
                console.error('[清除头像] 轮询日志失败:', e);
            }
        }

        /**
         * 添加来自跨域存储的日志条目
         * @param {Object} logEntry - 日志条目对象 { time, msg, type, timestamp }
         */
        addLogFromCrossDomain(logEntry) {
            if (!this.logContainer) return;

            const { time, msg, type } = logEntry;

            // 创建日志条目元素
            const logItem = document.createElement('div');
            logItem.className = `ca-log-item ca-log-${type || 'info'}`;
            logItem.textContent = `[${time}] ${msg}`;
         ...
         * 清除跨域日志数据
         */
        clearCrossDomainLogs() {
            deleteCrossDomainData(STORAGE_KEYS.LOG_DATA);
    ...
    async function executeClearAvatar() {
        ...
        try {
            ...
            // ==================== 步骤10：自动关闭弹窗 ====================
            // 检查是否是弹窗模式（通过跨域存储标记判断）
            const isPopupMode = loadCrossDomainData(STORAGE_KEYS.POPUP_MODE, false);
            log('弹窗模式标记: ' + isPopupMode);
            log('window.opener: ' + (window.opener ? '存在' : '不存在'));
            ...
            if (isPopupMode) {
                ...
                // 清除弹窗模式标记
                deleteCrossDomainData(STORAGE_KEYS.POPUP_MODE);
    ...
    function initTicketPage() {
        ...
        // 启动跨域日志轮询（实时同步GM工具页面的日志）
        uiInstance.startLogPolling();
        ...
        uiInstance.registerZoneCallback('clear', async (element) => {
            ...
            try {
                // 清除旧的跨域日志数据，确保只显示当前操作的日志
                uiInstance.clearCrossDomainLogs();
                ...
                // 设置弹窗模式标记（用于关闭时判断）
                saveCrossDomainData(STORAGE_KEYS.POPUP_MODE, true);
                log('已设置弹窗模式标记');

                // 最大化弹窗：铺满整个屏幕，效果类似新标签页
    ...
    function initGMToolPage() {
        ...
        if (taskData && isBanToolPage()) {
            ...
            window.clearAvatarLogger = (msg, type) => {
                ...
                // 同步日志到跨域存储，供工单页面的状态栏日志面板读取
                try {
                    // 读取现有日志数据
                    let logData = loadCrossDomainData(STORAGE_KEYS.LOG_DATA, []);
                    ...
                    // 限制日志数量，避免存储过大
                    if (logData.length > CONFIG.MAX_LOG_LINES) {
                        logData = logData.slice(-CONFIG.MAX_LOG_LINES);
                    }
                    // 保存到跨域存储
                    saveCrossDomainData(STORAGE_KEYS.LOG_DATA, logData);
                } catch (e) {
                    console.error('[清除头像] 同步日志失败:', e);
                }
            };
